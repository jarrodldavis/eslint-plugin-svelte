...
const template_lexer = moo.states({
  ...
  html_open: {
    _html_tag_name: { match: /[^\s/>]+/, push: "html_attributes" },
    _whitespace: { match: /\s+/, lineBreaks: true, push: "html_attributes" },
  ...
  html_attribute_name: {
     ...
     * quotes in HTML tags. If a quote is encountered by the lexer in an
     * attribute name, `moo` will throw an error since this token forbids single
     * and double quotes and there's no other token in this state that allows
     * them.
     *
