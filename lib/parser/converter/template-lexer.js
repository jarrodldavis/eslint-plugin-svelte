...
const template_lexer = moo.states({
  ...
  // used by all expressions to prevent early termination of mustache tags
  // used in each context expressions to prevent early start of each index expressions
  nested_brace: {
    _left_brace: { match: /{/, push: "nested_brace" },
    _right_brace: { match: /}/, pop: true },
    _brace_contents: { match: /[^{}]+/, lineBreaks: true }
  },
  // used in each expressions to prevent early termination of each expressions
  // used in each context expressions to prevent early start of each index expressions
  // used in await expressions to prevent early termination of await expressions
  nested_bracket: {
    _left_bracket: { match: /\[/, push: "nested_bracket" },
    _right_bracket: { match: /]/, pop: 1 },
    _bracket_contents: { match: /[^[\]]+/, lineBreaks: true }
  },
  // used in each expressions to prevent early termination of key expressions
  nested_paren: {
    _left_paren: { match: /\(/, push: "nested_paren" },
  ...
  // used by all expressions to prevent early termination of mustache tags
  // used in each expressions to prevent early termination each expressions
  // used in each context expressions to prevent early start of each index expressions
  // used in each key expressions to prevent early termination of each key expressions
  // used in await expressions to prevent early termination of await expressions
