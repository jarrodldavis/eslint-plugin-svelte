...
function convert(ast, code, options) {
  ...
  function offset(amount, positionable) {
    ...
    positionable.loc = source.get_loc(positionable.start, positionable.end);
    positionable[offset_marker] = true;
  ...
  function set_loc(node) {
    ...
    node.loc = node.loc || source.get_loc(start, end);
    return node;
  }
  ...
  function tokenize_expression_children(node) {
    ...
    if (node.type === "InlineComponent" && /[A-Z]/u.test(node.name[0])) {
      node.identifier = {
        type: "Identifier",
        name: node.name,
        start: node.start + 1,
